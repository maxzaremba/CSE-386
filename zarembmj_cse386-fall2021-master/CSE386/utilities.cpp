...
double min(double A, double B, double C) {
	/* CSE 386 - todo  */
	double smaller = std::fmin(A, B);
	return std::fmin(smaller, C);
}
...
double max(double A, double B, double C) {
	/* CSE 386 - todo  */
	double larger = std::fmax(A, B);
	return std::fmax(larger, C);
}
...
bool isOrthogonal(const dvec3 &a, const dvec3 &b) {
	/* CSE 386 - todo  */
	double cos = (glm::dot(a, b) / glm::length(a / b));
...
bool formAcuteAngle(const dvec3& a, const dvec3& b) {
	return ((glm::dot(a, b) / glm::length(a) * glm::length(b)) < 90);
}
...
double areaOfTriangle(const dvec3& pt1, const dvec3& pt2, const dvec3& pt3) {
	...
	dvec3 ab = (pt1, pt2);
	dvec3 ac = (pt1, pt3);
	return ((1 / 2) * areaOfParallelogram(ab, ac));
}
...
dvec3 pointingVector(const dvec3& pt1, const dvec3& pt2) {
	/* CSE 386 - todo  */
	dvec3 pointing = pt2 - pt1;
	return glm::normalize(pointing);
}
...
int quadratic(double A, double B, double C, double roots[2]) {
	...
	if (A == 0) {
		...
		roots[0] = (double)(-B + root) / (2 * A);
		roots[1] = (double)(-B - root) / (2 * A);
		std::sort(roots, roots + 2);
		return 2;
	}
	else if (D == 0) {
